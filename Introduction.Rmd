...
+ 2 **清洗（tidy）**数据  
将导入R内的数据清洗成一致的格式：每一列是一个变量，每一行是一个观测。

+ 3 **转换（transform）**数据  
在数据集中选取感兴趣的子集，构造新的变量，计算统计量。**tidying**和**transforming**统称为**wrangling**。

+ 4 数据**可视化（visualisation）**  
数据可视化可以展现数据中隐藏的规律，引发新的问题，提示你可能问了错误的问题，或者使用了错误的数据。

+ 5 数据**建模（model）**  
建模是对可视化的补充。如果能够将问题精确地限定，就可以通过建模回答这些问题。但是建模只能证实或否定假设，却无法对假设本身提出疑问。
